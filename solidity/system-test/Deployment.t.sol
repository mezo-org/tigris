...
contract Deployment is BaseSystemTest {
    ...
    function testPoolImplementation() public {
        assertNotEq(poolImplementation, address(0), "Pool implementation address should be non-zero");
    }
    ...
    function testGaugeFactory() public {
        assertNotEq(address(gaugeFactory), address(0), "GaugeFactory address should be non-zero");
    }

    function testVotingRewardsFactory() public {
        assertNotEq(address(votingRewardsFactory), address(0), "VotingRewardsFactory address should be non-zero");
    }

    function testManagedRewardsFactory() public {
        assertNotEq(address(managedRewardsFactory), address(0), "ManagedRewardsFactory address should be non-zero");
    }
    ...
    function testVeBTC() public {
        assertNotEq(address(veBTC), address(0), "VeBTC address should be non-zero");

        assertEq(veBTC.forwarder(), address(forwarder), "VeBTC forwarder should be properly set");
        assertEq(veBTC.token(), address(BTC), "VeBTC token should be properly set");
        assertEq(veBTC.factoryRegistry(), address(factoryRegistry), "VeBTC factory registry should be properly set");
        assertEq(veBTC.voter(), address(veBTCVoter), "VeBTC voter should be properly set");
        assertEq(veBTC.distributor(), address(veBTCRewardsDistributor), "VeBTC distributor should be properly set");
        assertEq(veBTC.team(), address(governance), "VeBTC team should be properly set");
    }

    function testVeBTCVoter() public {
        assertNotEq(address(veBTCVoter), address(0), "VeBTCVoter address should be non-zero");

        assertEq(veBTCVoter.forwarder(), address(forwarder), "VeBTCVoter forwarder should be properly set");
        assertEq(veBTCVoter.ve(), address(veBTC), "VeBTCVoter ve should be properly set");
    ...
    function testVeBTCRewardsDistributor() public {
        assertNotEq(address(veBTCRewardsDistributor), address(0), "VeBTCRewardsDistributor address should be non-zero");

        assertEq(address(veBTCRewardsDistributor.ve()), address(veBTC), "VeBTCRewardsDistributor ve should be properly set");
        assertEq(veBTCRewardsDistributor.depositor(), address(chainFeeSplitter), "VeBTCRewardsDistributor depositor should be properly set");
    }

    function testChainFeeSplitter() public {
        assertNotEq(address(chainFeeSplitter), address(0), "ChainFeeSplitter address should be non-zero");

        assertEq(address(chainFeeSplitter.voter()), address(veBTCVoter), "ChainFeeSplitter voter should be properly set");
        assertEq(address(chainFeeSplitter.token()), address(BTC), "ChainFeeSplitter token should be properly set");
        assertEq(address(chainFeeSplitter.rewardsDistributor()), address(veBTCRewardsDistributor), "ChainFeeSplitter rewards distributor should be properly set");
    }

    function testVeBTCEpochGovernor() public {
        assertNotEq(address(veBTCEpochGovernor), address(0), "VeBTCEpochGovernor address should be non-zero");

        assertTrue(veBTCEpochGovernor.isTrustedForwarder(address(forwarder)), "VeBTCEpochGovernor should consider the forwarder as trusted");
