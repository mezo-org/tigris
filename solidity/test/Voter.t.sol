...
contract VoterTest is BaseTest {
    ...
    function testCannotVoteForGaugeThatDoesNotExist() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IVoter.GaugeDoesNotExist.selector, fakePool)
        );
        voter.vote(1, pools, weights);
    }
    ...
    function testCannotWhitelistIfNotGovernor() public {
        vm.prank(address(owner2));
        vm.expectRevert(IVoter.NotGovernor.selector);
        voter.whitelistToken(address(LIMPETH), true);
    }
    ...
    function testKillGaugeWithRewards() public {
        ...
        // add deposit
        deal(address(pool), address(this), POOL_1);
        ...
        uint256 rebase = chainFeeSplitter.updatePeriod();
        assertGt(rebase, 0);
        voter.updateFor(address(gauge));

        uint256 reward = BTC.balanceOf(address(voter));
        assertGt(reward, 0);
        uint256 claimableBefore = voter.claimable(address(gauge));
        assertApproxEqRel(claimableBefore, reward, 1e6);

        assertEq(BTC.balanceOf(address(chainFeeSplitter)), 0);

        voter.killGauge(address(gauge));
        assertEq(voter.claimable(address(gauge)), 0);
        // Minimal remains from rounding
        assertLt(BTC.balanceOf(address(voter)), 1e2); // check for dust
        assertEq(BTC.balanceOf(address(chainFeeSplitter)), claimableBefore);

        // zero-out rewards from chainFeeSplitter so in a new rebase, new BTC lands
        uint256 splitterBalance = BTC.balanceOf(address(chainFeeSplitter));
        vm.prank(address(chainFeeSplitter));
        BTC.transfer(address(1), splitterBalance);
        ...
        // distribute rebase and sync gauge
        deal(address(BTC), address(chainFeeSplitter), TOKEN_1);
        rebase = chainFeeSplitter.updatePeriod();
        assertGt(rebase, 0);
        assertEq(voter.claimable(address(gauge)), 0);
        voter.updateFor(address(gauge));
        ...
        assertLt(BTC.balanceOf(address(voter)), 1e2); // check for dust
        assertGt(BTC.balanceOf(address(chainFeeSplitter)), 0);
    }
    ...
    function testCanStillDistributeAllWithKilledGauge() public {
        ...
        assertEq(voter.claimable(address(gauge)), 334999999999999999);
        assertEq(voter.claimable(address(gauge2)), 334999999999999999);
        ...
        assertEq(BTC.balanceOf(address(gauge)), 0);
    ...
    function testNotifyRewardAmount() public {
        ...
        deal(address(BTC), address(chainFeeSplitter), TOKEN_1);
        vm.prank(address(chainFeeSplitter));
        BTC.approve(address(voter), TOKEN_1);

        uint256 splitterPre = BTC.balanceOf(address(chainFeeSplitter));
        uint256 voterPre = BTC.balanceOf(address(voter));

        vm.prank(address(chainFeeSplitter));
        vm.expectEmit(true, false, false, true, address(voter));
        emit NotifyReward(address(chainFeeSplitter), address(BTC), TOKEN_1);
        voter.notifyRewardAmount(TOKEN_1);

        uint256 splitterPost = BTC.balanceOf(address(chainFeeSplitter));
        uint256 voterPost = BTC.balanceOf(address(voter));

        assertEq(voterPost - voterPre, TOKEN_1);
        assertEq(splitterPre - splitterPost, TOKEN_1);
    }
