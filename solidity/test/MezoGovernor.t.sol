...
contract MezoGovernorTest is BaseTest {
    ...
    function testCannotSetVetoerToZeroAddress() public {
        ...
        vm.expectRevert(MezoGovernor.ZeroAddress.selector);
        governor.setVetoer(address(0));
    }

    function testCannotSetVetoerIfNotVetoer() public {
        vm.prank(address(owner2));
        vm.expectRevert(MezoGovernor.NotVetoer.selector);
    ...
    function testCannotAcceptVetoerIfNotPendingVetoer() public {
        ...
        vm.expectRevert(MezoGovernor.NotPendingVetoer.selector);
    ...
    function testCannotVetoIfNotVetoer() public {
        ...
        calldatas[0] = abi.encodeWithSelector(
            voter.whitelistToken.selector,
            address(mUSD)
        );
        string memory description = "Whitelist mUSD";
    ...
    function testVetoProposal() public {
        ...
        assertGt(
            governor.getVotes(address(owner), 1, proposalStart),
            governor.quorum(proposalStart)
        ); // check quorum
        ...
        assertEq(
            ...
            uint256(IVetoGovernor.ProposalState.Vetoed)
        );
    ...
    function testCannotSetProposalNumeratorAboveMaximum() public {
        vm.expectRevert(MezoGovernor.ProposalNumeratorTooHigh.selector);
        governor.setProposalNumerator(501);
    }

    function testCannotSetProposalNumeratorIfNotTeam() public {
        vm.prank(address(owner2));
        vm.expectRevert(MezoGovernor.NotTeam.selector);
    ...
    function testCannotExecuteWithoutQuorum() public {
        ...
        assertEq(
            uint256(governor.state(pid)),
            uint256(IVetoGovernor.ProposalState.Pending)
        );
    ...
    function testProposalHasQuorum() public {
        ...
        assertEq(
            uint256(governor.state(pid)),
            uint256(IVetoGovernor.ProposalState.Active)
        );
        ...
        assertEq(
            uint256(governor.state(pid)),
            uint256(IVetoGovernor.ProposalState.Succeeded)
        );
        ...
        assertEq(
            uint256(governor.state(pid)),
            uint256(IVetoGovernor.ProposalState.Executed)
        );
    ...
    function testProposalHasQuorumWithDelegatedVotes() public {
        ...
        BTC.approve(address(escrow), TOKEN_1 * 100);
        ...
        assertEq(
            escrow.getPastVotes(address(owner), 3, block.timestamp),
            997253115368668515 + TOKEN_1 * 100
        );
    ...
    function testProposeWithUniqueProposals() public {
        ...
        uint256 pid2 = governor.propose(
            ...
            calldatas,
            description
        ); // will revert if pids not unique
