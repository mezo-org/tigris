...
contract ManagedNftTest is BaseTest {
    ...
    function testCreateManagedLockFor() public {
        ...
        assertEq(
            uint256(escrow.escrowType(mTokenId)),
            uint256(IVotingEscrow.EscrowType.MANAGED)
        );
    ...
    function testDepositManagedWithDelegatingManagedNFT() public {
        ...
        assertEq(
            escrow.getPastVotes(address(owner), tokenId2, 1213201),
            TOKEN_1 + 992437206566602005
        );
    ...
    function testWithdrawManagedWithLockedReward() public {
        ...
        assertEq(BTC.balanceOf(address(lockedManagedReward)), 0);
        freeManagedReward = FreeManagedReward(escrow.managedToFree(mTokenId));
        assertEq(BTC.balanceOf(address(freeManagedReward)), 0);
        ...
        assertEq(BTC.balanceOf(address(escrow)), TOKEN_1 * 2);
        assertEq(BTC.balanceOf(address(lockedManagedReward)), 0);
    }
    ...
    function testWithdrawManagedWithFreeReward() public {
        ...
        BTC.approve(address(freeManagedReward), TOKEN_1);
        freeManagedReward.notifyRewardAmount(address(BTC), TOKEN_1);

        assertEq(escrow.supply(), supply);
        assertEq(BTC.balanceOf(address(freeManagedReward)), TOKEN_1);
        assertEq(
            freeManagedReward.tokenRewardsPerEpoch(address(BTC), 604800),
            TOKEN_1
        );
    ...
    function testWithdrawManagedWithLockedRewardWithDelegatingManagedNFT()
        public
    {
        skip(1 hours);
        uint256 mTokenId = escrow.createManagedLockFor(address(owner2));
        ...
        assertEq(checkpoint.delegatee, 0);
        assertEq(
            escrow.getPastVotes(address(owner), tokenId2, 1818001),
            987642686019226005
        );
        assertEq(escrow.balanceOfNFTAt(tokenId2, 1818000), 987642693946674000);
        assertEq(
            escrow.getPastVotes(address(owner), tokenId2, 1818000),
            TOKEN_1 * 2 + 987642693946674000
        );
        ...
        assertEq(BTC.balanceOf(address(escrow)), TOKEN_1 * 3);
        assertEq(BTC.balanceOf(address(lockedManagedReward)), 0);
    }
