...
const incentiveTokens = [
  {
    ...
    decimals: 8,
    balance: '1.25',
    logoURI: 'data:image/svg+xml,' + encodeURIComponent(`
...
const IncentiveTokenOption = ({ token }: { token: typeof incentiveTokens[0] }) => (
...
const IncentivizeInterface = ({ pools } : { pools: Pool[] }) => {
  ...
  const selectedToken = incentiveTokens.find(t => t.address === selectedTokenAddress)

  const canProceed = selectedPool && selectedTokenAddress && amount && Number(amount) > 0
            ...
            <Select value={selectedTokenAddress} onValueChange={setSelectedTokenAddress}>
              <SelectTrigger>
                <SelectValue placeholder="Select a token">
                  {selectedToken && <IncentiveTokenOption token={selectedToken} />}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
              ...
              <Alert variant="warning">
                <AlertDescription>
                  ...
                  <ul className="text-sm space-y-1">
                    <li>Pool: {selectedPoolData?.token0.symbol}/{selectedPoolData?.token1.symbol}</li>
                    <li>Amount: {amount} {selectedToken?.symbol}</li>
                    <li>Warning: Adding incentives is permanent. You won't be able to recover these tokens.</li>
                  </ul>
                </AlertDescription>
              </Alert>
